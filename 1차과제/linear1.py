...
# forward elimination
 # 1. 초기 pivot의 위치는 [0, 0]으로 한다.
  ...
  {
  if (matrix[:,j] == is_nearzero);
  {
  j++;
  }
  if (matrix[i,j] == is_nearzero);
  interchange(matrix, row_i, row_i+1);

  scaling(matrix, row_i, matrix[i,j]);

  replacement(matrix, row_i+1, row_i, matrix[i+1, j]);
...
def is_zerorow(matrix, ith_row):  
	m, n = matrix.shape  for r in range(0, n): 
...
def rank(aug_mat):  m, n = aug_mat.shape  
rank_A = 0  
rank_M = 0  
...
elif (rank_A = rank_M && rank_A == n);
FE_result = A unique solution;
else
FE_result = Infinity many soluyion;
